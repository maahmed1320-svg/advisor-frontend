...
function toMins(t) {
  ...
  const parts = t.trim().split(/\s*-\s*(?=\d)/)
    ...
    <div className={s.page}>
      ...
      <div className={s.topbar}>
        ...
        <span className={s.subtitle}>
          {cartItems.length} course{cartItems.length !== 1 ? 's' : ''} · {totalCr} credits
        </span>
        {hasConflicts && <span className={s.conflictPill}>⚠ Time conflict</span>}
      </div>
